<script lang="ts">
  ...
  import { songs, type SpotifyTrack, spotifyTracks } from "$lib/store";
  ...
  let spotifyUser: SpotifyApi.CurrentUsersProfileResponse;
        ...
        await Promise.all(
          ...
          response.map(({ tracks }, i) => {
            if (tracks.items.length) {
              const { external_urls, album, artists, ...rest } =
                tracks.items[0];
              return {
                ...rest,
                ...currentSongs[i],
...
{#if isTokenValid}
  {#await getSpotifySongs()}
    <p>Loading your Spotify songs...</p>
  {:then _}
    <p>
      Successfully loaded {
        get(spotifyTracks).filter(
          (track) => track !== null,
        ).length
      } Spotify tracks!
    </p>
  {:catch error}
    <p>Error loading Spotify songs: {error.message}</p>
  {/await}
{:else}
